...
def inicializar_locks(manager, posicoes_baterias, robos_array):
    """
    ...
    # Cria um lock para cada posição de bateria
    for posicao_bateria in posicoes_baterias:
        key = f"{posicao_bateria[0]}{posicao_bateria[1]}"
        baterias_dict_mutex[key] = manager.Lock()

    # Cria um lock para cada robô baseado no ID
    for robo in robos_array:
        key = str(robo.id)
        robos_dict_mutex[key] = manager.Lock()

    return baterias_dict_mutex, robos_dict_mutex
...
def realizar_acao_player(robo,tabuleiro,nova_posicao,baterias_dict_mutex):
    ...
    if valor_tabuleiro == 0:  # Espaço livre
        ...
        pass  # Implementar lógica de coleta de bateria e organizacao dos mutex se necessário
    elif valor_tabuleiro == 99:  # Robô jogador
        pass  # Implementar lógica de interação entre robôs se necessário